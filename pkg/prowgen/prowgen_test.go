...
func TestGenerateJobs(t *testing.T) {
	tests := []struct {
		...
		},
		{
			...
			config: &ciop.ReleaseBuildConfiguration{
				Tests: []ciop.TestStepConfiguration{
					{As: "publish", Postsubmit: true, MaxConcurrency: ptr.To(4), ContainerTestConfiguration: &ciop.ContainerTestConfiguration{From: "bin"}},
				},
			},
			repoInfo: &ProwgenInfo{Metadata: ciop.Metadata{
				Org:    "organization",
		...
		},
		{
			...
			config: &ciop.ReleaseBuildConfiguration{
				Tests: []ciop.TestStepConfiguration{
					{As: "nightly", Cron: ptr.To(cron), MaxConcurrency: ptr.To(2), ContainerTestConfiguration: &ciop.ContainerTestConfiguration{From: "bin"}},
				},
			},
			repoInfo: &ProwgenInfo{Metadata: ciop.Metadata{
				Org:    "organization",
